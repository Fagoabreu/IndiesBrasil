...
describe("GET /api/v1/user", () => {
  ...
  describe("Default user", () => {
    test("With valid session", async () => {
      ...
      // Valor corrigido: antes era `no-store,no-cache-max-age=0,must-revalidate`
      // (diretiva inválida, `no-cache-max-age=0`).
    ...
    test("With halfway-expired session", async () => {
      ...
      expect(Math.abs(expiresAt - updatedAt - session.EXPIRATION_IN_MILLISECONDS)).toBeLessThanOrEqual(5000);
