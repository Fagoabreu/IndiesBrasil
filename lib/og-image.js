...
/**
 * Selo arredondado — tipo do evento, "Cancelado", etc.
 *
 * A caixa é desenhada à parte porque o librsvg não tem `border-radius` para
 * texto: a pílula é o retângulo, o texto só fica dentro. A largura vem da
 * estimativa de avanço dos caracteres (ver `estimateTextWidth`), então o selo
 * acompanha o texto sem precisar medir de verdade.
 *
 * `variant: "solid"` pinta o fundo cheio com o texto em branco. Serve para o
 * selo que precisa se destacar ao lado dos outros: dois selos contornados de
 * cores parecidas (o tipo e "Cancelado" são ambos vermelhos, por exemplo) leem
 * como um bloco só.
 */
export function svgPill({ text, x, y, height = 42, fontSize = 22, accent, letterSpacing = 1.5, variant = "outline" }) {
  const width = pillWidth(text, fontSize, letterSpacing);
  const radius = height / 2;
  const solid = variant === "solid";

  const box = solid
    ? `<rect x="${x}" y="${y}" width="${width}" height="${height}" rx="${radius}" fill="${accent}"/>`
    : `<rect x="${x}" y="${y}" width="${width}" height="${height}" rx="${radius}" fill="${accent}" fill-opacity="0.12" stroke="${accent}" stroke-opacity="0.35" stroke-width="1.5"/>`;

  return [
    box,
    `<text x="${x + width / 2}" y="${y + height * 0.68}" fill="${solid ? "#ffffff" : accent}" font-size="${fontSize}" font-weight="bold" font-family="${SVG_FONT_FAMILY}" text-anchor="middle" letter-spacing="${letterSpacing}">${escapeXml(String(text ?? "").toUpperCase())}</text>`,
  ].join("");
}

/**
 * Largura de um selo, para quem precisa posicionar o próximo ao lado.
 *
 * O `letterSpacing` entra na conta: sem ele, um selo com espaçamento entre
 * letras sairia com o texto encostando na borda e o vizinho por cima.
 */
function pillWidth(text, fontSize, letterSpacing) {
  const label = String(text ?? "").toUpperCase();
  const paddingX = Math.round(fontSize * 0.9);
  return Math.round(estimateTextWidth(label, fontSize) + label.length * letterSpacing + paddingX * 2);
}

/**
 * Vários selos em sequência, cada um com a largura do próprio texto.
 *
 * Evita que quem chama calcule à mão onde começa o segundo selo — o
 * deslocamento depende do texto do primeiro, que só é conhecido aqui dentro.
 *
 * @param {{ items: Array<{ text: string, accent: string, variant?: string }> }} opts
 */
export function svgPillRow({ items, x, y, gap = 14, height = 42, fontSize = 22, letterSpacing = 1.5 }) {
  let cursor = x;

  return items
    .map((item) => {
      const svg = svgPill({ ...item, x: cursor, y, height, fontSize, letterSpacing });
      cursor += pillWidth(item.text, fontSize, letterSpacing) + gap;
      return svg;
    })
    .join("");
}
