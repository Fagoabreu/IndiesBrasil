...
/**
 * Linha de atribuição: avatar pequeno seguido de `@username`.
 *
 * É o que identifica o autor sem virar o protagonista do card. No card de post o
 * assunto é a publicação — a identidade grande, com avatar e `@` em destaque,
 * pertence ao card de perfil (`pages/api/og/profile/[username].js`); repetir
 * aquela composição aqui fazia a prévia do post parecer a prévia do perfil,
 * desenhada por cima da imagem da publicação.
 *
 * `y` é o centro vertical da linha; o grupo fica centralizado na zona segura.
 */
export function svgAuthorRow({ avatarUrl, username, y, avatarSize = 48, fontSize = 28, gap = 14 }) {
