...
async function checkTwitchChannels(channelNames) {
  ...
  if (!process.env.TWITCH_CLIENT_ID || !process.env.TWITCH_CLIENT_SECRET) {
    console.warn(
      "[streamChecker] TWITCH_CLIENT_ID ou TWITCH_CLIENT_SECRET n\u00e3o configurados. Canais Twitch n\u00e3o ser\u00e3o verificados.",
    );
    return [];
  }
...
async function checkYouTubeChannels(channelIds) {
  ...
  if (!process.env.YOUTUBE_API_KEY) {
    console.warn(
      "[streamChecker] YOUTUBE_API_KEY n\u00e3o configurada. Canais YouTube n\u00e3o ser\u00e3o verificados.",
    );
    return [];
  }
  ...
  for (const channelId of channelIds) {
    try {
      ...
      const videoRes = await fetch(
        `${YOUTUBE_VIDEOS_URL}?part=liveStreamingDetails,snippet&id=${videoId}&key=${process.env.YOUTUBE_API_KEY}`,
      );
      ...
      if (videoRes.ok) {
        ...
        viewerCount = item?.liveStreamingDetails?.concurrentViewers
          ? Number.parseInt(item.liveStreamingDetails.concurrentViewers, 10)
          : null;
      }
