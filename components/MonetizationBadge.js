...
export default function MonetizationBadge({
  ...
  if (hasLootboxes) {
    flags.push({
      ...
      label: "Lootboxes",
      title:
        "Este jogo contém lootboxes (caixas de recompensa aleatória pagas) — Lei Felca PL 412/2022",
    });
  }
  ...
  if (hasInGamePurchases) {
    flags.push({
      ...
      label: "Compras",
      title:
        "Este jogo contém compras dentro do jogo (microtransações) — Lei Felca PL 412/2022",
    });
  }
  ...
  return (
    <span
      className={`${BASE_CLASS} ${BASE_CLASS}--${size}`}
      aria-label="Selos de transparência de monetização — Lei Felca"
    >
      {flags.map((flag) => (
        <span
          key={flag.key}
          className={`${styles.badge} ${styles[flag.key]}`}
          title={flag.title}
        >
          {flag.icon}
