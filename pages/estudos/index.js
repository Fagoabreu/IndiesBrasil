...
import { BookIcon, PlusIcon, StarFillIcon } from "@primer/octicons-react";
...
export default function CursosPage() {
  ...
  async function loadCourses(pageNum, searchQuery, tagFilter) {
    setLoading(true);
    try {
    ...
    <div className={styles.page}>
      ...
      <header className={styles.pageHeader}>
        <div className={styles.headerBlock}>
          ...
          <p className={styles.pageSubtitle}>Aprenda com a comunidade indie brasileira.</p>
      ...
      {!loading && courses.length === 0 && (
        <div className={styles.emptyState} role="status" aria-live="polite">
          ...
          <p className={styles.emptyDescription}>{search || tag ? "Tente outro termo ou filtro." : "Seja o primeiro a criar um curso!"}</p>
        </div>
      )}
