...
router.get(controller.canRequest("update:studio"), listHandler);
router.post(controller.canRequest("update:studio"), createHandler);
...
async function assertCanEdit(studio, requestUser) {
  ...
  if (!isAdmin && studio.owner_id !== requestUser.id) {
    throw new ForbiddenError({ message: "Apenas administradores do estúdio podem gerenciar contatos." });
  }
}
...
async function createHandler(request, response) {
  ...
  if (!contact_type_id || !contact_value?.trim()) {
    throw new ValidationError({ message: "Tipo e valor do contato são obrigatórios." });
  }
