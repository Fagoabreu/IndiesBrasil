...
/**
 * POST /api/v1/boardgames/[slug]/images
 *
 * Body: FormData com
 *   file    — arquivo de imagem
 *   imgType — "cover" | "banner"
 *
 * Apenas admins e o dono do jogo de mesa podem fazer upload.
 */
  ...
  try {
    ...
    if (!canEdit) {
      throw new ForbiddenError({
        message: "Apenas administradores do estúdio podem alterar as imagens do jogo de mesa.",
      });
    }
    ...
    const folder = `boardgames/${boardgameData.id}/${imgType}`;
    const imageData = await uploadedImages.uploadImage(file, folder);

    if (imgType === "cover") {
      await boardgame.saveCover(slug, imageData.id);
    } else {
      await boardgame.saveBanner(slug, imageData.id);
    }
